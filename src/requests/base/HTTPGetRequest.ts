...
export default abstract class HTTPGetRequest extends HTTPRequest {
    ...
    constructor(
        ...
        if (options) {
            ...
            this.multiFormat = options.multiFormat
                ? options.multiFormat
                : false;
        }
    }
    ...
    public validateFormatParam() {
        if (this._req.query["format"]) {
            const formatString: string = this._req.query["format"]
                ?.toString()
                .toLowerCase();
    ...
    private validatePagination() {
        ...
        if (!(page == undefined && pageSize == undefined)) {
            if (page == undefined || pageSize == undefined) {
                throw new BadSyntaxException(
                    "For pagination of response, both page and pageSize query parameters must be included",
                );
            } else {
                ...
                if (isNaN(pageNum) || isNaN(pageSizeNum)) {
                    throw new BadSyntaxException(
                        "Query parameters page and pageSize must be valid numbers",
                    );
                } else {
    ...
    public async run() {
        try {
            ...
            if (exception instanceof HTTPException) {
                ...
                // If the object ID cast failed
                const notFound: ResourceNotFoundException =
                    new ResourceNotFoundException(exception.value);
    ...
    protected sendResponse(documents: NonNullable<unknown>) {
        ...
        const format: string | undefined =
            this.multiFormat && this._req.query["format"]
                ? this._req.query["format"].toString().toLowerCase()
                : undefined;
        if (format == "csv") {
