...
export default class DiscordAuthService {
  ...
  public async getTokenInfo(code: string): Promise<IDiscordTokenInfo> {
    ...
    try {
      const tokenResponse = await axios.post<IDiscordTokenInfo>(
        `${DiscordAuthService.API_ENDPOINT}/oauth2/token`,
      ...
      if (err.isAxiosError) {
        ...
        if (axiosError.response) {
          throw new ServerErrorException(
            `Discord token request failed with code ${
              axiosError.response.status
            }:\n ${JSON.stringify(axiosError.response.data)}`
          );
        } else {
          throw new ServerErrorException(
            `Discord token request failed with code ${axiosError.code}`
          );
        }
      } else {
  ...
  public async getUserInfo(
    tokenInfo: IDiscordTokenInfo
  ): Promise<IDiscordUserInfo> {
    const options: AxiosRequestConfig = {
      headers: {
    ...
    try {
      const userResponse = await axios.get<IDiscordUserInfo>(
        `${DiscordAuthService.API_ENDPOINT}/users/@me`,
        options
      );
      ...
      if (err.isAxiosError) {
        ...
        if (axiosError.response) {
          throw new ServerErrorException(
            `Discord User Info Request failed with code ${
              axiosError.response.status
            }:\n ${JSON.stringify(axiosError.response.data)}`
          );
        } else {
          throw new ServerErrorException(
            `Discord User Info Request failed with code ${axiosError.code}`
          );
        }
      } else {
        throw new ServerErrorException(err?.message);
